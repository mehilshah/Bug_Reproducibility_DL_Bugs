...
from tensorflow.keras.layers import AveragePooling2D, Lambda, Conv2D, Conv2DTranspose, Activation, concatenate, BatchNormalization, ZeroPadding2D
...
def Upsample(tensor, size):
    ...
    def bilinear_upsample(x, size):
        resized = tf.image.resize(images=x, size=size, method='bilinear')
        return resized
...
def DeepLabV3Plus(img_height, img_width, nclasses=66):
    ...
    base_model = ResNet50(input_shape=(img_height, img_height, 3), weights='imagenet', include_top=False)
