...
X_train = np.random.rand(4000, 22)
X_test = np.random.rand(1000, 22)
y_train = np.ones((4000, 1))
y_test = np.ones((1000, 1))

X_train_t = torch.tensor(X_train).float()
X_test_t = torch.tensor(X_test).float()
y_train_t = torch.tensor(y_train).long().reshape(X_train_t.shape[0], 1)
y_test_t = torch.tensor(y_test).long().reshape(X_test_t.shape[0], 1)
