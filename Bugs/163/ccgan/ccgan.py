...
class CCGAN():
    ...
    def train(self, epochs, batch_size=128, sample_interval=50):
        ...
        X_train = np.array([tf.image.resize(image, [self.img_rows, self.img_cols]).numpy() for image in X_train])
    ...
    def save_model(self):
        def save(model, model_name):
            model_path = f"saved_model/{model_name}.json"
            weights_path = f"saved_model/{model_name}_weights.h5"
            options = {"file_arch": model_path, "file_weight": weights_path}
            json_string = model.to_json()
            with open(options['file_arch'], 'w') as file:
